...
def print_solution(data, presentation):
    ...
    for vehicle_id in range(len(presentation)):
        nowload = 0
        route_distance = 0
        tmpstr = 'Route {0} - Vehicle: {1} KGS'.format(vehicle_id,new_cap[vehicle_id])
        result.append(tmpstr)
        tmpstr = ''
        tmpstr += 'From {0} to {1}:'.format(0,presentation[vehicle_id][0])
        tmpstr += ' Weight: {0} KGS | Distance: {1} km'.format(demand[presentation[vehicle_id][0]],distance_mt[0][presentation[vehicle_id][0]])
        result.append(tmpstr)
        j =0
        for j in range(len(presentation[vehicle_id])):
            ...
            if (j != 0):
                tmpstr +='From {0} to {1}:'.format(presentation[vehicle_id][j-1],presentation[vehicle_id][j])
                tmpstr += ' Weight: {0} KGS | Distance: {1} km'.format(demand[presentation[vehicle_id][j]],distance_mt[presentation[vehicle_id][j-1]][presentation[vehicle_id][j]])
                result.append(tmpstr)
        ...
        total_load += nowload
        tmpstr = ''
        tmpstr +='From {0} to {1}:'.format(presentation[vehicle_id][j],0)
        tmpstr += ' Weight: {0} KGS | Distance: {1} km'.format(0,distance_mt[presentation[vehicle_id][j]][0])
        result.append(tmpstr)        
        tmpstr = 'Distance of the Route: {} km'.format(route_distance)
        result.append(tmpstr)
        tmpstr = 'Total transport capacity: {} kg'.format(nowload)
        result.append(tmpstr)
        tmpstr = "                                     "
        result.append(tmpstr)
    ...
    for i in range(len(dt)):
        if dt[i] != 0:
            for v in range(dt[i]):
                vehicle_id+=1
                tmpstr = 'Route {0} - Vehicle: {1} KGS'.format(vehicle_id,data['list_cap'][0])
                result.append(tmpstr)
                tmpstr = 'From {0} to {1}: Weight: {2} KGS | Distance: {3} km'.format(0,i,data1['list_cap'][0],data1['distance_matrix'][0][i])
                result.append(tmpstr)
                tmpstr = 'From {0} to {1}: Weight: {2} KGS | Distance: {3} km'.format(i,0,0,data1['distance_matrix'][i][0])
                ...
                result.append(tmpstr)
                tmpstr = 'Total transport capacity: {} kg'.format(data1['list_cap'][0])
                result.append(tmpstr)
                total_distance += rt
                tmpstr = '                              '
                result.append(tmpstr)
    ...
    for index_cell in range(1,10):
        ...
        if (index_cell != 8  and index_cell != 1):
            ...
            space_cell.value = "Total Distance"
        else:
    ...
    tmpstr = "Total number of vehicles:"
    result.append(tmpstr)
    for i in range(size_cap):
        if tke_xe[i] != 0:
    ...
    tmp_str2 = 'Total transport capacity of all routes: {} kg'.format(total_load)
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow,widget_names):
        ...
        self.pushButton3.setGeometry(QtCore.QRect(570, 120, 160, 23))
        self.pushButton3.setObjectName("pushButton3")
        self.pushButton4 = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton4.setGeometry(QtCore.QRect(290, 97, 100, 25))
        ...
        self.comboBox1.setGeometry(QtCore.QRect(30, 97, 251, 23))
        self.comboBox1.setObjectName("comboBox1")
        for i in widget_names:
            self.comboBox1.addItem("")
        #
        self.label_5 = QtWidgets.QLabel(self.centralwidget)
        self.label_5.setGeometry(QtCore.QRect(30, 80, 80, 13))
        # icon gk
        self.label_6 = QtWidgets.QLabel(self.centralwidget)
        self.label_6.setGeometry(QtCore.QRect(610,20,90,60))
        self.label_6.setPixmap(QtGui.QPixmap("data/gk-logistics-logo.png"))
        self.label_6.setScaledContents(True)
        self.label_6.setObjectName("photo")
        # link gk
        self.label_7 = QtWidgets.QLabel(self.centralwidget)
        self.label_7.setGeometry(QtCore.QRect(620,80,90,20))
        self.label_7.linkActivated.connect(self.link)
        self.label_7.setText('<a href="http://www.gk-logistics.com.vn/">GK LOGISTICS</a>')
        # link team profile
        self.label_8 = QtWidgets.QLabel(self.centralwidget)
        self.label_8.setGeometry(QtCore.QRect(610,100,90,20))
        self.label_8.linkActivated.connect(self.link)
        self.label_8.setText('<a href="https://bitly.com.vn/subyo5">About Developers</a>')
        #
        self.label = QtWidgets.QLabel(self.centralwidget)
        self.label.setGeometry(QtCore.QRect(30, 30, 150, 13))
        self.label.setObjectName("label")
        self.label_2 = QtWidgets.QLabel(self.centralwidget)
        self.label_2.setGeometry(QtCore.QRect(290, 30, 170, 13))
        self.label_2.setObjectName("label_2")
        self.spinBox = QtWidgets.QSpinBox(self.centralwidget)
        self.spinBox.setGeometry(QtCore.QRect(290, 50, 100, 22))
        self.spinBox.setObjectName("spinBox")
        self.spinBox.setRange(0,100000)
        self.label_3 = QtWidgets.QLabel(self.centralwidget)
        self.label_3.setGeometry(QtCore.QRect(30, 125, 101, 16))
        self.label_3.setObjectName("label_3")
        #search bar
        self.pushButton2 = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton2.setGeometry(400,120,75,23)
        # #scroll area
    ...
    def retranslateUi(self, MainWindow,widget_names):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "OPTI-HAQA  -  Sponsored by GK Logistics Viet Nam"))
        self.pushButton.setText(_translate("MainWindow", "Select"))
        self.pushButton3.setText(_translate("MainWindow", "Add Place"))
        self.pushButton4.setText(_translate("MainWindow", "Change depot"))
        ...
        self.label.setText(_translate("MainWindow", "Customer's Name: "))
        self.label_2.setText(_translate("MainWindow", "Weight (kg):"))
        self.label_3.setText(_translate("MainWindow", "Request List:"))
        self.label_5.setText(_translate("MainWindow", "Depot's Name:"))
...
class OnOffWidget(QWidget):
    def __init__(self,name):
        ...
        self.btn_on = QPushButton("Update") #The ON button
        self.btn_off = QPushButton("Delete") #the off button
...
class AnotherWindow1(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.label1 = QLabel("New Customer:")
        self.label7 = QLabel("Name:")
        self.lineEdit5 = QLineEdit()
        self.label2 = QLabel("Longitude and Latitude:")
    ...
    def addOnePoint(self):
        ...
        #
        check_empty = 0
        if (isnan(sheet_obj1.cell(row=1,column=2).value)):
            print(1)
        ...
        if check_empty != 1:
            ...
            sheet_obj1.cell(row = 1 , column = 1).value = nameOfPoint
            sheet_obj1.cell(row = 1 , column = 2).value = latitude
            sheet_obj1.cell(row = 1 , column = 3).value = longitude
            sheet_obj2.cell(row= 3, column = 1).value = nameOfPoint
            sheet_obj2.cell(row= 1, column = 3).value = nameOfPoint
            sheet_obj2.cell(row=3 , column =  3).value = 0
        wb_obj1.save("data/coordinate.xlsx")
        wb_obj2.save("data/data.xlsx")
        global or_data
        or_data = create_data_model()
